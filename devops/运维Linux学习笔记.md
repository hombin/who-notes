...
6. 切换用户
   ...
   # 指定 [用户名] 切换
   su - [用户名]
   # 普通用户切换到 root 用户需要密码，该密码是普通用户的密码
   # 不加 - 是不改变当前变量
   su [用户名]
   ```
...
4. at 命令
   ...
   ```shell
   # 在特定的时间执行一次性的任务 ("at>" 写入执行命令，Ctrl+D 退出并保存)
   at
   ...
   -t：后面<时间参数> 以时间参数的形式提交要运行的任务 [-t CCYYMMDDhhmmss]
...
3. dump 实例
    ...
    # 将/home目录所有内容备份到 /tmp/homeback.bak 文件中
    # 备份层级为 0 并在/etc/dumpdates中记录相关信息
